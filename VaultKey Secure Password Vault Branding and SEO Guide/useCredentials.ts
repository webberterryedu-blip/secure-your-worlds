...
import { encrypt, decrypt, getDerivedKey, deriveKey, generateSalt, uint8ArrayToBase64Url, base64UrlToUint8Array, clearKeyCache } from "@/lib/crypto";
...
export function useCredentials() {
    ...
    queryFn: async () => {
      ...
      const decryptedCredentials = await Promise.all(data.map(async (cred) => {
        ...
        try {
          ...
          const cryptoKey = await getDerivedKey(masterPassword, saltUint8);
  ...
  const addMutation = useMutation({
    mutationFn: async (cred: CredentialInsert) => {
      ...
      const cryptoKey = await getDerivedKey(masterPassword, salt);
